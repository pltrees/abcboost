...
This toolkit consists of ABCBoost, a concrete implementation of [ABCBoost](https://arxiv.org/pdf/0811.1250.pdf) (Adaptive Base Class Boost) and other Gradient Tree Boosting algorithms including [MART](https://projecteuclid.org/euclid.aos/1013203451) (Multiple Additive Regression Trees) and [Robust LogitBoost](https://projecteuclid.org/euclid.aos/1016218223). Details on usage are elaborated as follows.
...
To build ABCBoost with multithreading support, install [OpenMP](https://en.wikipedia.org/wiki/OpenMP) (OpenMP comes with the system GCC toolchains on Linux) and set the option `OMP=ON`:
```
cmake -DOMP=ON ..
make
```
...
```
Notice that only the train file is required for training. To train the model with 100 iterations, launch `train` from the command line:
```
./train -data data/mimage.train.libsvm -J 4 -v 0.1 -iter 100
```
where `-J 4` specifies 4 terminal node for each tree, `-v 0.1` represents the shrinkage rate, a.k.a., learning rate, is 0.1.
Two files are generated in the working directory:
* The saved model we just trained: `mimage.train.libsvm_abcrobustlogit_J4_v0.1.model`
...
```
./train -data data/mimage.train.libsvm -J 4 -v 0.1 -iter 100 -method mart
```
<strong>Regression:</strong>
Regression problems are supported by adding `-method regression`:
```
./train -data data/comp_cpu.train.libsvm -J 4 -v 0.1 -iter 100 -method regression
```
...
```
./train -data data/mslr10k.train -query data/mslr10k.train.query -J 4 -v 0.1 -iter 100 -method lambdarank
./predict -data data/mslr10k.test -query data/mslr10k.test.query -model mslr10k.train_lambdarank_J4_v0.1.model
```
...
```
./predict -data data/mimage.test.libsvm -model mimage.train.libsvm_abcrobustlogit_J4_v0.1.model
```

To test the first 50 iterations of the trained model (the trained model has 100 iterations):
```
./predict -data mimage.test.libsvm -model mimage.train.libsvm_abcrobustlogit_J4_v0.1.model -iter 50
```
### Parameter Tuning

Here we illustrate some common parameters and provide some examples:
* `-iter` number of iterations (default 100)
* `-J` number of leaves in a tree (default 4)
* `-v` learning rate (default 0.08)
...
* `-n_threads` number of threads (default 1) <strong>It can only be used when multithreading is enabled. (Compile the code with `-DOMP=ON` in cmake.)</strong>
...
To train the model with 1000 iterations, 8 leaves per tree and 0.01 learning rate:
```
./train -data data/mimage.train.libsvm -iter 1000 -J 8 -v 0.01
```

To train the model with 1000 iterations, 8 leaves per tree, 0.01 learning rate and enable the exhaustive base class searching (10 is the number of classes of the mimage.train.libsvm dataset):
```
./train -data data/mimage.train.libsvm -iter 1000 -J 8 -v 0.01 -search 10
```
Note that the exhaustive searching produces better-generalized model while requiring substantially more time.

To train the model with 1000 iterations, 8 leaves per tree, 0.01 learning rate and also use the value of the test data to quantize the data:
```
./train -data data/mimage.train.libsvm -iter 1000 -J 8 -v 0.01 --additional_files data/mimage.test.libsvm
```
The labels in the specified additional files are not used in the training. Only the feature values are used to generate better quantization. Better testing results may be obtained when using additional files
...
When computing the base base, we either support heuristic search strategy or introduce gaps (where we update the choice of the base class only for every G boosting steps) to speed up learning.
...
### Bagging
We provide bagging option which selects a random sample of the training set at each boosting step and fits trees to these samples. We also support feature bagging which selects a random subset of the features at each candidate split in the learning process. Sampling rates can be set with the `--model_data_sample_rate` and `--model_feature_sample_rate`parameter.
...
```
./predict -data data/mimage.train.libsvm -model mimage.train.libsvm_abcrobustlogit_J4_v0.06.model
```
